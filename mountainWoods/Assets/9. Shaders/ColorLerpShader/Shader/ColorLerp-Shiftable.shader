	...
	SubShader
	{
		...
		Pass
		{
			...
			fixed4 frag (v2f i) : SV_Target
			{
				...
				fixed lumVal = Luminance(allColors);

				fixed pixelColLerpVal = min(max(_FadeRange - length(_PlayerPosition.xyz - i.worldPos) * 0.5, 0), 1) ;
				fixed4 col = lerp(lumVal, allColors, pixelColLerpVal);
